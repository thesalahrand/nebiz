  ...
  <form method="post" action="{{ route('stores.services.update', ['store' => $store->id, 'service' => $service->id]) }}"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
      ...
      <div>
        ...
        <x-text-input id="price" name="price" type="number" :value="old('price', $service->price)" step="0.01" autocomplete="price"
          min="0" max="999999.99" />
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
      <div>
        ...
        <x-text-input id="duration" name="duration" type="number" :value="old('duration', $service->duration)" step="0.01"
      ...
      <div>
        ...
        <x-select-input id="duration_unit_name" name="duration_unit_name" :options="App\Enums\DurationUnit::toCollection()"
          chooseOptionText="Select a unit name" :selectedOptionValue="old('duration_unit_name', $service->duration_unit_name)" autocomplete="duration_unit_name" />
        <x-input-error :messages="$errors->get('duration_unit_name')" />
      </div>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
      <div x-data="imageViewer(@js($service->getFirstMediaUrl('service-photos', 'thumb') ?: asset('images/placeholder-image.png')))">
      ...
      <div>
        ...
        <x-textarea-input rows="9" id="additional_info" name="additional_info" autocomplete="additional_info"
          maxlength="1000"
          placeholder="{{ __('Additional information about your store..') }}">{{ old('additional_info', $service->additional_info) }}</x-textarea-input>
